...
class Role extends Base
{
    ...
    {
        $data = \app\admin\model\Role::where(["id"=>["neq",1]])->paginate(10);
        $this->assign([
            "data" => $data
        ]);
        return $this->fetch();
    ...
    public function saveView()
    {
        ...
        $helper = new \app\common\controller\Helper();
        $data = $privilege->field("id,name,parent_id")->select();
        $rdata = (new \app\admin\model\Role)->where(["id" => $id])->find();
        //使用树型结构展示上级权限
        $treeArr = $helper->get_tree($data);
        $this->assign([
            "treeArr" => $treeArr,
    ...
    public function save()
    {
        ...
        $data["privilege_list"] = implode(",", $data["privilege_list"]);
        if (!\app\admin\model\Role::update($data)) {
    ...
    {
        $id = $this->request->post("id");
        if(!\app\admin\model\Role::destroy($id)){
